...
const TaskForm: React.FC = () => {
  ...
  // modalの開閉状態の管理。初期は閉じた状態
  const [open, setOpen] = useState(false);
  // modalのstyle
  const [modalStyle] = useState(getModalStyle);
  // modalで入力されるテキストを管理する
  ...
  // stringもnumberも受け取れるようにしておく
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value: string | number = e.target.value;
    const name = e.target.name;
    if (name === "estimate") {
      // estimateを編集した際はnumber型にキャストする
      dispatch(editTask({ ...editedTask, [name]:  Number(value) }));
    }
    dispatch(editTask({ ...editedTask, [name]: value }));
  };

  // 担当者を変更したときに呼び出される。eventのvalueは型指定できないため、unkownで受け取る
  const handleSelectRespChange = (e: React.ChangeEvent<{ value: unknown }>) => {
    // 担当者はnumber型になるため、本来は非推奨だが、型アサーションを使用している。
  ...
  // userの一覧をリストで表示させる
  const userOptions = users.map((user) => ( // userの数だけ、menuItemを作成する
    <MenuItem key={ user.id } value={ user.id }>
      { user.username }
    </MenuItem>
  ));
  // categoryの一覧をリストで表示させる
    ...
    <div>
      ...
      <form>
        <TextField
          ...
          type="number"
          name="estimate"
          InputProps={{ inputProps: { min: 0, max: 1000 } }} // minusを入力させない
          InputLabelProps={{
            shrink: true,
          }}
